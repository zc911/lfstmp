...
int impSobelX_Abs_U8(uchar *pubyImg, int dwW, int dwH, uchar *pubySblX) {
    ...
    for (dwRI = 1; dwRI < dwH - 1; dwRI++) {
        ...
        for (dwCI = 2; dwCI < dwW - 2; dwCI++) {
            if (pubyRow2[dwCI] * pubyRow2[dwCI - 1] * pubyRow2[dwCI + 1] * pubyRow2[dwCI - 2] * pubyRow2[dwCI + 2]
                == 0) {
                continue;
            }
            wVal = (pubyRow1[dwCI + 1] + pubyRow2[dwCI + 1] * 2 + pubyRow3[dwCI + 1])
                - (pubyRow1[dwCI - 1] + pubyRow2[dwCI - 1] * 2 + pubyRow3[dwCI - 1]);
            wVal = abs(wVal) / 4;
            if (wVal > 255) {
                wVal = 255;
            }
...
int doRectify_f(float *pfImage0,
                float *pfImage1,
                int dwW,
                int dwH,
                float fAngle_old,
...
//  cout << "doRectify_f 1\n";
    afPnts[0 * 3 + 0] = adwPolygonXY[0 * 2 + 0];
    afPnts[0 * 3 + 1] = adwPolygonXY[0 * 2 + 1];
    afPnts[0 * 3 + 2] = 1.0f;
    afPnts[1 * 3 + 0] = adwPolygonXY[1 * 2 + 0];
    afPnts[1 * 3 + 1] = adwPolygonXY[1 * 2 + 1];
    afPnts[1 * 3 + 2] = 1.0f;
    afPnts[2 * 3 + 0] = adwPolygonXY[2 * 2 + 0];
    afPnts[2 * 3 + 1] = adwPolygonXY[2 * 2 + 1];
    afPnts[2 * 3 + 2] = 1.0f;
    afPnts[3 * 3 + 0] = adwPolygonXY[3 * 2 + 0];
    afPnts[3 * 3 + 1] = adwPolygonXY[3 * 2 + 1];
    afPnts[3 * 3 + 2] = 1.0f;
...
//  cout << "doRectify_f 4\n";
    *pfAngle_new = fAngle_new;
    ...
    adwPolygonXY[0 * 2 + 0] = pfNewPnts[0 * 2 + 0];
    adwPolygonXY[0 * 2 + 1] = pfNewPnts[0 * 2 + 1];
    adwPolygonXY[1 * 2 + 0] = pfNewPnts[1 * 2 + 0];
    adwPolygonXY[1 * 2 + 1] = pfNewPnts[1 * 2 + 1];
    adwPolygonXY[2 * 2 + 0] = pfNewPnts[2 * 2 + 0];
    adwPolygonXY[2 * 2 + 1] = pfNewPnts[2 * 2 + 1];
    adwPolygonXY[3 * 2 + 0] = pfNewPnts[3 * 2 + 0];
    adwPolygonXY[3 * 2 + 1] = pfNewPnts[3 * 2 + 1];
