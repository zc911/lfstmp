...
//      cout << "dwSize:" << dwSize << ", pstRPNImgNow->dwImgW:" << pstRPNImgNow->dwImgW << ", " << "pstRPNImgNow->dwImgH:" << pstRPNImgNow->dwImgH << endl;
            ...
            for (dwRI = dwY0, dwRI2 = 0; dwRI < dwY1; dwRI++, dwRI2++) {
                memcpy(pstRPNImgNow->pfData + dwRI2 * pstRPNImgNow->dwImgW,
                       pfDataSrc + dwRI * dwImgWSrc + dwX0,
                       sizeof(float) * pstRPNImgNow->dwImgW);
            }
        }
    }
//  cout << "fuck rpn 1\n";
...
//      cout << "H:" << lprect.fHeight << ", W:" << lprect.fWidth << endl;
            LPRectInfo lprect_crop(lprect.fScore,
                                   lprect.fCentY,
                                   lprect.fCentX,
                                   lprect.fHeight * afCropSize[1],
                                   lprect.fWidth * afCropSize[0]);
...
//      cout << adwMarginHW[0] << ", " << adwMarginHW[1] << endl;
            ...
            for (dwRI = 0; dwRI < dwH_0; dwRI++) {
                memcpy(pfBlkBuffer_0 + (dwRI + adwMarginHW[0]) * dwBlkW + adwMarginHW[1],
                       pfImage + (dwRI + dwY0_0) * dwImgW + dwX0_0,
                       sizeof(float) * dwW_0);
            }

            LPRectInfo lprect_new(lprect.fScore,
                                  lprect.fCentY,
                                  lprect.fCentX,
                                  lprect.fHeight * afNewSize[1],
                                  lprect.fWidth * afNewSize[0]);
...
int doRecogOneRow(LPDR_HANDLE hChRecog,
...
int doRectifyWithPolyReg
    (LPDR_HANDLE hPolyReg, InputInfoRecog_S *pstIIR, int adwMRatioXY[2], float fAngle_old, float *pfAngle_new);
int doRecogOne(LPDR_HANDLE handle, InputInfoRecog_S *pstIIR, LPDRInfo_S *pstOut) {
    int dwRet = 0;
    int dwI, dwJ;
...
//    cout << dwI << endl;
    ...
    if (abs(dwSepY - rect_new.dwY0 + 1) * 4 > abs(rect_new.dwY1 - rect_new.dwY0 + 1)) {
        ...
        rectUp.dwX0 = rect_new.dwX0;
        rectUp.dwY0 = rect_new.dwY0;
        rectUp.dwX1 = rect_new.dwX1;
        rectUp.dwY1 = dwSepY + dwYAdd;

        rectDwn.dwX0 = rect_new.dwX0;
        rectDwn.dwY0 = dwSepY;
        rectDwn.dwX1 = rect_new.dwX1;
        rectDwn.dwY1 = rect_new.dwY1;
...
//    if (!dwRet)
        {
            ...
            dwStep = 4;
            dwRetDn =
                doRecogOneRow(hChRecog, &stImage, rects_out[1], fStrechRatio, fShrinkRatio, dwStep, 6.0f, &stOut1);
...
int mainlandLPCheck(LPDRInfo_S *pstOut) {
    ...
    if (pstOut->dwLPLen >= 7) {
        dwNowV0 = pstOut->adwLPNumber[0];
        dwNowV1 = pstOut->adwLPNumber[1];
        if (dwNowV0 >= 11 && dwNowV0 <= 35
            && ((dwNowV1 >= 11 && dwNowV1 <= 35) || dwNowV1 == 1 || dwNowV1 == 2)) //military or goverment case
        {
        }
...
int doRectifyWithPolyReg(LPDR_HANDLE hPolyReg,
                         InputInfoRecog_S *pstIIR,
                         int adwMRatioXY[2],
                         float fAngle_old,
                         float *pfAngle_new) {
    int dwRI;
    ...
    adwPolygonOut[0 * 2 + 0] += dwCrop_X0;
    adwPolygonOut[0 * 2 + 1] += dwCrop_Y0;
    adwPolygonOut[1 * 2 + 0] += dwCrop_X0;
    adwPolygonOut[1 * 2 + 1] += dwCrop_Y0;
    adwPolygonOut[2 * 2 + 0] += dwCrop_X0;
    adwPolygonOut[2 * 2 + 1] += dwCrop_Y0;
    adwPolygonOut[3 * 2 + 0] += dwCrop_X0;
    adwPolygonOut[3 * 2 + 1] += dwCrop_Y0;
    adwPolygonOut[4 * 2 + 0] += dwCrop_X0;
    adwPolygonOut[4 * 2 + 1] += dwCrop_Y0;
    adwPolygonOut[5 * 2 + 0] += dwCrop_X0;
    adwPolygonOut[5 * 2 + 1] += dwCrop_Y0;

    doRectify_f(pfImage_0, pfImage_1, dwImgW, dwImgH, fAngle_old, adwPolygonOut, pfAngle_new);

    LPRect &rectnow = pstIIR->rect;
    rectnow.dwX0 = adwPolygonOut[0];
    rectnow.dwY0 = adwPolygonOut[1];
    rectnow.dwX1 = adwPolygonOut[0];
    rectnow.dwY1 = adwPolygonOut[1];
...
//          else if (fV < 0.3)
            ...
            if ((adwIdxs[0] == 0 || adwIdxs[0] == 8 || adwIdxs[0] == 5)
                && (adwIdxs[1] == 2 || adwIdxs[1] == 6 || adwIdxs[1] == 5)
                && adwColorsHist[0] * 40 < adwColorsHist[1] * 100) {
