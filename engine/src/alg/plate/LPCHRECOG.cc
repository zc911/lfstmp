...
int LPCHRECOG_Create(LPDRModel_S stCHRECOG, int dwDevType, int dwDevID, LPDR_HANDLE *phCHRECOG) {
    ...
    *phCHRECOG = (LPDR_HANDLE) pstModule;
    ...
    ret = MXNDArrayLoadFromBytes(stCHRECOG.pbyParam,
                                 stCHRECOG.dwParamSize,
                                 &param_size,
                                 &paramh_arr,
                                 &param_name_size,
                                 &param_names);
...
int LPCHRECOG_Process(LPDR_HANDLE hCHRECOG,
    ...
    for (dwRI = 0; dwRI < dwHCrop; dwRI++) {
        memcpy(pfDataCrop + dwRI * dwWCrop + dwBlackMargin,
               pfDataSrc + (dwRI + rect.dwY0) * dwImgWSrc + rect.dwX0,
               sizeof(float) * (dwWCrop - 2 * dwBlackMargin));
    }
