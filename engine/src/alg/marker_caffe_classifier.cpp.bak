...
vector<vector<Detection> > MarkerCaffeClassifier::ClassifyBatch(
     ...
     for (map<int, Marker>::iterator it = markers_.begin();
          ...
          vector<vector<Detection> > rpns = get_final_bbox(
                    tiny_images, cls, reg, enlarge_ratios, it->second, imgs);
          for (int i = 0; i < batch_size; i++) {
...
vector<vector<Detection> > MarkerCaffeClassifier::ClassifyAutoBatch(
     ...
     for (auto batch_images : PrepareBatch(images, caffe_config_.batch_size)) {
          vector<vector<Detection> > pred = ClassifyBatch(
                    batch_images);
...
vector<vector<Detection> > MarkerCaffeClassifier::get_final_bbox(
          ...
          vector<float> enlarge_ratios, Marker &marker,
     ...
     for (int idx = 0; idx < vvbbox.size(); idx++) {
          sort(vvbbox[idx].begin(), vvbbox[idx].end(), mycmp);
          nms(vvbbox[idx], marker.threshold);
          vector<Detection> final_vbbox;
          for (int i = 0; i < vvbbox[idx].size(); i++) {

               if (!vvbbox[idx][i].deleted
                         && vvbbox[idx][i].confidence > marker_config_.global_confidence) {
                    ...
                    if (final_vbbox.size() > marker.max)
                         break;
                    if (filter(box, origin_imgs[idx].rows,
                               origin_imgs[idx].cols))
