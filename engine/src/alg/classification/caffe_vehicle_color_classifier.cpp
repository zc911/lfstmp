...
vector<vector<Prediction> > CaffeVehicleColorClassifier::ClassifyAutoBatch(const vector<Mat> &imgs) {
...
vector<vector<Prediction> > CaffeVehicleColorClassifier::ClassifyBatch(
...
vector<Blob<float> *> CaffeVehicleColorClassifier::PredictBatch(
    const vector<Mat> imgs) {
...
void CaffeVehicleColorClassifier::WrapBatchInputLayer(
    std::vector<std::vector<cv::Mat> > *input_batch) {
    Blob<float> *input_layer = net_->input_blobs()[0];
...
void CaffeVehicleColorClassifier::PreprocessBatch(
