...
PlateRecognizer::PlateRecognizer(const PlateConfig &config){
        ...
        nRet = TH_SetProvinceOrder((char *)config.LocalProvince.c_str(),&c_Config);
...
int PlateRecognizer::recognizeImage(const Mat &img) {
     Mat sample;
     unsigned char pImg[img.rows * img.cols * 3];
     ...
     int nRet = TH_RecogImage(pImg, width, height, &result, &nResultNum, NULL,
               &c_Config);
...
vector<Vehicle::Plate> PlateRecognizer::RecognizeBatch(const vector<Mat> &imgs) {
     if (nRet != 0) {
