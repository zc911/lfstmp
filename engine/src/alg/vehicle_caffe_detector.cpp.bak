...
VehicleCaffeDetector::VehicleCaffeDetector(CaffeConfig &config)
...
int VehicleCaffeDetector::DetectBatch(const vector<Mat> & batch,
...
vector<Blob<float>*> VehicleCaffeDetector::PredictBatch(vector<Mat> imgs) {
     if (!device_setted_) {
...
void VehicleCaffeDetector::WrapBatchInputLayer(
          std::vector<std::vector<cv::Mat> > *input_batch) {
     Blob<float>* input_layer = net_->input_blobs()[0];
...
void VehicleCaffeDetector::PreprocessBatch(
