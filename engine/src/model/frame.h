...
public:
	Frame(const Identification id) :
			id_(id), timestamp_(0), status_(FRAME_STATUS_INIT), payload_(0)
	{
	}

	Frame(const Identification id, unsigned int width, unsigned int height,
			unsigned char *data) :
			id_(id), timestamp_(0), status_(FRAME_STATUS_INIT)
	{
		payload_ = new Payload(id_, width, height, data);
	}

	Frame(const Identification id, cv::Mat img) :
			id_(id), timestamp_(0), status_(FRAME_STATUS_INIT)
	{
