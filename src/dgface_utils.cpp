...
bool dimConsist(const vector <RecogResult> &recog_results) {
    for (size_t i = 0; i < recog_results.size(); ++i) {
        ...
        if (curr_recog.face_feat.empty()) {
            ...
            LOG(ERROR) << "feature dimension not match" << endl;
            return false;
        }
    }
    return true;
...
bool loadFeature(const string &fea_log, vector <RecogResult> &recog_results) {
    ...
    if (!feafile) {
        LOG(ERROR) << "can't open " << fea_log << endl;
        return false;
    }
...
float computeAP(const vector<float> &scores, const vector<bool> &trues) {
    if (scores.size() != trues.size()) {
        LOG(ERROR) << "score size and label size not match." << endl;
        return -2;
    }
    ...
    std::sort(score_label_vec.begin(),
              score_label_vec.end(),
              [](const pair<float, bool> &left, const pair<float, bool> &right) {
...
float computeMAP(const vector <vector<float>> &score_vec,
                 const vector <vector<bool>> &true_vec,
                 vector<float> &AP_vec) {
    ...
    if (cnt > 0) {
        ...
        LOG(ERROR) << "AP list is empty." << endl;
        return -1;
    }
}
