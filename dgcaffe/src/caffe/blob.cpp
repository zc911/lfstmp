...
void Blob<Dtype>::Reshape(const vector<int>& shape) {
  ...
  for (int i = 0; i < shape.size(); ++i) {
    CHECK_GE(shape[i], 0);
    if (count_ != 0) {
      CHECK_LE(shape[i], INT_MAX / std::max(1, count_)) << "blob size exceeds INT_MAX";
    }
    count_ *= shape[i];
    shape_[i] = shape[i];
...
template <typename Dtype>
void Blob<Dtype>::ReshapeHaHa(const vector<int>& shape) {
  CHECK_LE(shape.size(), kMaxBlobAxes);
  count_ = 1;
  shape_.resize(shape.size());
  if (!shape_data_ || shape_data_->size() < shape.size() * sizeof(int)) {
...
template <typename Dtype>
void Blob<Dtype>::ReshapeLikeHaHa(const Blob<Dtype>& other) {
  //const BlobShape& shape = other.shape();
  //CHECK_LE(shape.dim_size(), kMaxBlobAxes);
  //vector<int> shape_vec(shape.dim_size());
  //for (int i = 0; i < shape.dim_size(); ++i) {
  //  shape_vec[i] = shape.dim(i);
  //}
  //ReshapeHaHa(shape_vec);
  ReshapeHaHa(other.shape());
}
