...
def start(input_file_path, output_file_path):
	...
	if file:
		...
		while line:
			...
			if(len(items) < 2):
				...
				return
			file_path = items[0]
			file_path = file_path[file_path.rindex("/") + 1:]
			feature_value = items[1].replace("\n", "")
			output.write(str(id) + " " + name + " " + file_path + " " + feature_value + " {}\n" )
			line = file.readline()
...
if __name__ == "__main__":
	if len(sys.argv) != 3 :
		print("usage: "+ sys.argv[0]+ " $FEATURE_FILE $FORMATED_FEATURE_FILE")
		exit(0)
