...
class ServerBase {
 public:
    ...
    class Request {
        ...
        class iequal_to {
         public:
            bool operator()(const std::string &key1,
            ...
            try {
                remote_endpoint_address =
                        socket.lowest_layer().remote_endpoint().address()
                                .to_string();
                remote_endpoint_port = socket.lowest_layer().remote_endpoint()
                        .port();
    ...
    std::unordered_map<std::string,
            std::unordered_map<std::string,
                    std::function<
                            void(typename ServerBase<socket_type>::Response&,
                                 std::shared_ptr<
                                         typename ServerBase<socket_type>::Request>)> > > resource;

    std::unordered_map<std::string,
            std::function<
                    void(typename ServerBase<socket_type>::Response&,
                         std::shared_ptr<
                                 typename ServerBase<socket_type>::Request>)> > default_resource;

 private:
    std::vector<
            std::pair<std::string,
                    std::vector<
                            std::pair<boost::regex,
                                    std::function<
                                            void(typename ServerBase<socket_type>::Response&,
                                                 std::shared_ptr<
                                                         typename ServerBase<
                                                                 socket_type>::Request>)> > > > > opt_resource;
    ...
    void start() {
        ...
        for (auto& res : resource) {
            for (auto& res_method : res.second) {
                ...
                for (auto opt_it = opt_resource.begin();
                        opt_it != opt_resource.end(); opt_it++) {
                ...
                it->second.emplace_back(boost::regex(res.first),
                                        res_method.second);
            }
        }
        ...
        if (config.address.size() > 0)
            endpoint = boost::asio::ip::tcp::endpoint(
                    boost::asio::ip::address::from_string(config.address),
                    config.port);
        else
            endpoint = boost::asio::ip::tcp::endpoint(
 ...
 protected:
    ...
    ServerBase(unsigned short port, size_t num_threads, size_t timeout_request,
    ...
    std::shared_ptr<boost::asio::deadline_timer> set_timeout_on_socket(
            std::shared_ptr<socket_type> socket, size_t seconds) {
    ...
    bool parse_request(std::shared_ptr<Request> request,
        ...
        if ((method_end = line.find(' ')) != std::string::npos) {
            size_t path_end;
            if ((path_end = line.find(' ', method_end + 1))
                    != std::string::npos) {
                ...
                if ((protocol_end = line.find('/', path_end + 1))
                        != std::string::npos) {
                    if (line.substr(path_end + 1, protocol_end - path_end - 1)
                            != "HTTP")
                        return false;
                    request->http_version = line.substr(
                            protocol_end + 1, line.size() - protocol_end - 2);
                } else
                    return false;
        ...
        for (auto& res : opt_resource) {
            if (request->method == res.first) {
                for (auto& res_path : res.second) {
                    boost::smatch sm_res;
                    if (boost::regex_match(request->path, sm_res,
                                           res_path.first)) {
    ...
    void write_response(
        ...
        boost::asio::spawn(
                request->strand,
                [this, &resource_function, socket, request, timer](boost::asio::yield_context yield) {
...
class Server<HTTP> : public ServerBase<HTTP> {
 public:
    Server(unsigned short port, size_t num_threads = 1, size_t timeout_request =
                   5,
