...
{
public:
    RestfulService(string protocol = "HTTP/1.1", string mime_type = "application/json; charset=utf-8")
            : protocol_(protocol)
...
protected:
    template <class request_type, class response_type>
    static void bind(HttpServer& server, string endpoint, string method, std::function<bool(const request_type*, response_type*)> func)
    {
        server.resource[endpoint][method] = [func](HttpServer::Response& response, std::shared_ptr<HttpServer::Request> request)
        {
