...
void serveWitness(Config *config, int userPort = 0) {
    ...
    std::thread springThread(&SpringGrpcClientImpl::Run, client);
...
DEFINE_bool(showconfig,
false, "Show config file content");
DEFINE_bool(encrypt,
false, "Use the encrype data, only valid in DEBUG mode");
