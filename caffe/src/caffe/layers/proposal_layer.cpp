...
//	std::cout << "shift x:" << shift_x.size() << std::endl;
//	std::cout << "shift x:" << "--------------------------" << std::endl;
    ...
    float hh_timecost = (time_hh.tv_sec - time_start.tv_sec) + double(time_hh.tv_usec - time_start.tv_usec) / 1.0e6;
    //std::cout << "hh timecost of proposal layer is: " << hh_timecost << std::endl;
    ...
    //std::cout << "info[0]: " << im_info[0] << std::endl;
    //std::cout << "info[1]: " << im_info[1] << std::endl;
    //std::cout << "info[2]: " << im_info[2] << std::endl;
	


    //cv::imshow("debug", cvimg);
    //cv::waitKey(-1);


	const vector< int > & shape0 = bottom[0]->shape();
	const vector< int > & shape1 = bottom[1]->shape();
	//const vector< int > & shape2 = bottom[2]->shape();
...
//	// generate anchors
    ...
    std::vector<std::vector<int> > anchor = {
                           { -20.75, -7.,35.75, 22.},
	                       {-32.65863992, -13.21320344, 47.65863992, 28.21320344},
	                       {-49.5, -22., 64.5, 37.},
	                       {-73.31727984, -34.42640687, 88.31727984, 49.42640687},
	                       {-107., -52., 122., 67.},
	                       {-154.63455967, -76.85281374, 169.63455967, 91.85281374},
	                       {-222.,-112.,237.,127.},
	                       { -12.,-12.,27.,27.},
	                       { -20.28427125,-20.28427125,35.28427125,35.28427125},
	                       {-32.,-32.,47.,47.},
                           {-48.56854249, -48.56854249,63.56854249,63.56854249},
                           {-72,-72,87,87},
                           {-105.13708499,-105.13708499,120.13708499,120.13708499},
                           {-152,-152,167,167},
                           {-5.75, -19.5, 20.75, 34.5},
                           {-11.44543648, -30.89087297, 26.4454364, 45.89087297}, 
                           {-19.5, -47., 34.5, 62.},
                           {-30.89087297, -69.78174593, 45.89087297, 84.78174593},
                           {-47., -102., 62., 117.},
                           {-69.78174593, -147.56349186, 84.78174593, 162.56349186},
                           {-102., -212., 117., 227.}
                           };
    
    //int scales[7] = {0, 1, 2, 3, 4, 5, 6};
    //float ratios[3] = {0.5, 1, 2.0};
    //float base_cx = 7.5, base_cy = 7.5;
    //for(int ratio_id = 0; ratio_id <= 2; ratio_id ++) {
    //    for(int scale_id = 0; scale_id <= 6; scale_id ++) {
    //       float scale = 40.0 * pow(2.0, scales[scale_id] / 2.0);
    //       float ratio = ratios[ratio_id];
    //       float w, h;
    //       w = round(sqrt(scale * scale * ratio));
    //       h = round(w / ratio);
    //       float lx, ly, rx, ry;
    //       lx = base_cx - w / 2.0;
    //       ly = base_cy - h / 2.0;
    ...
    //       //anchor.push_back(std::vector<int>{lx+0.5, ly+0.5, rx+0.5, ry+0.5});
    //       anchor.push_back(std::vector<int>{round(ly), round(lx), round(ry), round(rx)}); 
    //    } 
    //}
	...
	//std::cout << "feat_stride: " << feat_stride << std::endl;
	...
	int count_y = 0;
//	std::cout << "shift:";
    //std::cout << "x_l*y_l: " << x_l * y_l << std::endl;
...
//	std::vector<int> unit = { -84, -40,   99,   55};
//	test[0] = unit;
    
    //std::cout << "K" << K << " A" << A << std::endl;
	for(int i = 0; i<K*A;i++){
		anchor_t[i] = anchor[i%A];
//		std::cout << "{" << shifted_anchor[i][0] << "," << shifted_anchor[i][1] << "," << shifted_anchor[i][2] << "," << shifted_anchor[i][3] << std::endl;
	}
	int count_s = -1;
	for(int i=0; i<K*A; i++){
		if(i%A == 0){
	...
	//std::cout << "anchor size:" << anchor_t.size() << std::endl;	
	for(int i =0; i< K*A; i++){
		for(int j = 0; j<4; j++){
    ...
    float pre_timecost = (time_pre.tv_sec - time_start.tv_sec) + double(time_pre.tv_usec - time_start.tv_usec) / 1.0e6;
    //std::cout << "pre timecost of proposal layer is: " << pre_timecost << std::endl;
	
	std::vector<std::vector<Dtype> > bbox_deltas_ (count_1/4, vector<Dtype>(4));
	...
	for(int i = 0; i < count_1/4 ; i++){
		...
		for(int j = 0; j < 4; j++){
			...
			bbox_deltas_[i][j] = bbox_deltas[period2 + (count_all2 % shape1[1]) * H_W2];
			count_all2 += 1;
		}
		...
		dx = bbox_deltas_[i][0]; dy = bbox_deltas_[i][1]; dw = bbox_deltas_[i][2]; dh = bbox_deltas_[i][3];
        ...
        //if (i < 10) {
        //    std::cout << "lx: " << pred_ctr_x - 0.5 * pred_w << std::endl; 
        //    std::cout << "ly: " << pred_ctr_y - 0.5 * pred_h << std::endl; 
        //    std::cout << "rx: " << pred_ctr_x + 0.5 * pred_w << std::endl; 
        //    std::cout << "ry: " << pred_ctr_y + 0.5 * pred_h << std::endl; 
        //}
		// clip_boxes
		proposals[i][0] = std::max(std::min(Dtype(pred_ctr_x - 0.5 * pred_w), Dtype(im_info[1] -1)),Dtype(0));
		proposals[i][1] = std::max(std::min(Dtype(pred_ctr_y - 0.5 * pred_h), Dtype(im_info[0] -1)),Dtype(0));
		proposals[i][2] = std::max(std::min(Dtype(pred_ctr_x + 0.5 * pred_w), Dtype(im_info[1] -1)),Dtype(0));
		proposals[i][3] = std::max(std::min(Dtype(pred_ctr_y + 0.5 * pred_h), Dtype(im_info[0] -1)),Dtype(0));
		//if (i < 10) {
        //    std::cout << "lx: " << proposals[i][0] << std::endl; 
        //    std::cout << "ly: " << proposals[i][1] << std::endl; 
        //    std::cout << "rx: " << proposals[i][2] << std::endl; 
        //    std::cout << "ry: " << proposals[i][3] << std::endl; 
        //}
    ...
    //std::cout << "proposals_ size:" << proposals_.size() << std::endl;
    float mid_timecost = (time_mid.tv_sec - time_start.tv_sec) + double(time_mid.tv_usec - time_start.tv_usec) / 1.0e6;
    //std::cout << "mid timecost of proposal layer is: " << mid_timecost << std::endl;
	...
	* take top pre_nms_topN(6,000)
    ...
    //for(int i = 0; i < 10; i++) {
    //    std::cout << "score[" << i << "]: " << scores_[i] << std::endl;
    //}
    ...
    /*
    for (size_t i = idx.size()-1; i != idx.size()-1001; --i){
		proposals_f.push_back(proposals_[idx[i]]);
		scores_f.push_back(scores_[idx[i]]);
        //if(idx.size() - i < 10) {
        //    std::cout << "index i: " << i << std::endl;
    	//    std::cout << "{" << proposals_[idx[i]][0] << "," \
    	//    		<< proposals_[idx[i]][1] << "," \
    	//    		<< proposals_[idx[i]][2] << "," \
    	//    		<< proposals_[idx[i]][3] << "," \
        //            << scores_[idx[i]] << "}" << std::endl;
        //}
    }*/
...
//    	areas.push_back((proposals_f[i][2] - proposals_f[i][0] + 1)*(proposals_f[i][3] - proposals_f[i][1] + 1));
//    }
...
//    std::vector<Dtype> ovr(proposals_f.size());
    // 100 here!
    int count_temp = 0;
    while(proposals_tmp.size() > 0 && count_temp< 100){
    	...
    	for(int i = 1; i < proposals_tmp.size(); i++){
    		...
    		inter = w * h;
    		ovr = inter / ((proposals_tmp[0][2] - proposals_tmp[0][0] + 1)*(proposals_tmp[0][3] - proposals_tmp[0][1] + 1) \
    				+ (proposals_tmp[i][2] - proposals_tmp[i][0] + 1)*(proposals_tmp[i][3] - proposals_tmp[i][1] + 1) - inter);
    		if (ovr <= nms_thresh){
    ...
    //std::cout << "proposals_nms size:" << proposals_nms.size() << std::endl;
    
    //std::cout << "nms size:" << proposals_nms.size() << ";" << proposals_nms[0].size() << std::endl;
    //for (int i = 0; i < proposals_nms.size() ; i++){
    //	for (int j = 0; j < 4; j++){
    //		std::cout << proposals_nms[i][j] << " ";
    //	}
    //    std::cout << std::endl;
    //}
    ...
    //top[0]->Reshape(proposals_nms.size(), 5, 1, 1);
    ...
    for(int i = 0; i < proposals_nms.size(); i++){
        ...
        //std::cout << "nms 0: " << proposals_nms[i][0] << std::endl;
        //std::cout << "nms 1: " << proposals_nms[i][1] << std::endl;
        //std::cout << "nms 2: " << proposals_nms[i][2] << std::endl;
        //std::cout << "nms 3: " << proposals_nms[i][3] << std::endl;
        //cv::rectangle(cvimg, Rect(top_data[i*5 + 1], top_data[i*5 + 2], top_data[i*5 + 3] - top_data[i*5 + 1], top_data[i*5 + 4] - top_data[i*5 + 2]), (255,0,0));
    }
    ...
    float timecost = (time_end.tv_sec - time_start.tv_sec) + double(time_end.tv_usec - time_start.tv_usec) / 1.0e6;
    //std::cout << "timecost of proposal layer is: " << timecost << std::endl;
}
